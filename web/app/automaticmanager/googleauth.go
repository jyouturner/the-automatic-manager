...
func HandlerGoogleAuth(ctx *gin.Context) {
	session := sessions.Default(ctx)
	profile := session.Get("profile")
...
func getAuthCodeUrl(orgId string, userId string) ([]byte, error) {
	data := fmt.Sprintf(`
	{
		"user": {
			"org_id": "%s",
	...
	url := "https://6oz5xqe1cj.execute-api.us-west-2.amazonaws.com/dev/google"
